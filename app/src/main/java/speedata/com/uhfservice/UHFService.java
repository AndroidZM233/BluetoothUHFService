...
public class UHFService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (bluetooth ==null){
            Toast.makeText(UHFService.this, "该设备不支持蓝牙", Toast.LENGTH_SHORT).show();
            onDestroy();
        }
...
//                        readCard();
    ...
    private volatile long chenkTime = 0L;
    ...
    class MyReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (currentTimeMillis - chenkTime < 1000) {
                return;
            }
            chenkTime = currentTimeMillis;
            ...
            if ("0".equals(id)) {
                ...
                if ("low".equals(status)) {
                    status2 = 1;
                    sharedXmlUtil.write("status2", "1");
                    broadcastHandler.sendMessage(broadcastHandler.obtainMessage(10));
                } else {
                    status2 = 0;
                    sharedXmlUtil.write("status2", "0");
                }
            }
            if (status != null) {
                Log.d(TAG, "onReceive: status is:" + status);
                result = "N:" + GPI + "," + ReaderID + "," + seq + ",2," + status1 +
                        "," + status2 + "," + timeStyle2 + "\r\n";
                sendMsg(result);
            } else {
...
//        uhfService.CloseDev();
    ...
    //查询GPI状态 G（Gpi）
    public void getG() {
        String status1 = sharedXmlUtil.read("status1", "0");
        String status2 = sharedXmlUtil.read("status2", "0");
        if (!TextUtils.isEmpty(status1)) {
            result = "1:" + GPI + ",2," + status1 + "," + status2 + "\r\n";
        } else {
            result = "0:" + GPI + ",2," + status1 + "," + status2 + "\r\n";
        }
        sendMsg(result);
